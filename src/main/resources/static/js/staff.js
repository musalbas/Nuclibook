...
var validateEditForm = function (formObject) {
	...
	// check password length
	if (formObject.password != undefined && formObject.password != "" && formObject.password.length < 6) {
		toastr.error("Your password must be at least 6 characters long");
		error = true;
	}

	// check password match
	if ((formObject.password != undefined && formObject.password_check != undefined) && (formObject.password != formObject.password_check)) {
		toastr.error("Your passwords did not match");
		error = true;
	}
...
$(document).ready(function () {
	setUpDataTable('#staff-table', [[1, 1], [1, 1], [1, 1], [0, 0]], {
		order: [1, 'asc']
	});
});