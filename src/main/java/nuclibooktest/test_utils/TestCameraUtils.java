...
public class TestCameraUtils extends AbstractUtilTest{
    ...
    public void testGetCameraType(){
        ...
        assertTrue(camera.getId() == 1);
        assertNull(CameraUtils.getCamera(20));
    }

    @Test
    public void testGeAllCameras() throws Exception {
        assertTrue(CameraUtils.getAllCameras(false).size() == 2);
        assertTrue(CameraUtils.getAllCameras(true).size() == 1);
    }

    @Test
    public void testGetCamerasByCameraType(){
        //List<Camera> cameras = CameraUtils.getCamerasByCameraType()
        //assertTrue(StaffAvailabilityUtils.getAllStaffAvailabilities().size() == 2);
    }

    //todo get camera by therapy
    @Test
